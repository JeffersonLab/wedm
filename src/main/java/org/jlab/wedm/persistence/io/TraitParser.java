...
public class TraitParser {
  ...
  public static EDLFont parseFont(Map<String, String> traits, String key, EDLFont defaultValue) {
    ...
    try {
      if (value != null) {
        ...
        f =
            new EDLFont(
                name, "bold".equals(weight), "o".equals(oblique) || "i".equals(oblique), size);
      }
    } catch (Exception e) {
      LOGGER.log(
          Level.FINEST, "Unable to parse font; key: {0}; value: {1}", new Object[] {key, value});
    }
    return f;
  }
  ...
  public static boolean parseBoolean(Map<String, String> traits, String key) {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINEST, "Unable to parse Boolean; key: {0}; value: {1}", new Object[] {key, value});
    }

    return result;
  }

  public static EDLColor parseColor(
      Map<String, String> traits, ColorPalette palette, String key, EDLColor defaultValue) {
    String value = traits.get(key);
    ...
    try {
      ...
      LOGGER.log(
          Level.FINEST, "Unable to parse color; key: {0}; value: {1}", new Object[] {key, value});
    }
  ...
  public static Float parseFloat(Map<String, String> traits, String key, Float defaultValue) {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINEST, "Unable to parse int; key: {0}; value: {1}", new Object[] {key, value});
    }
      ...
      if (value != null) {
        ...
        for (int i = 0; i < lines.length; i++) {
          ...
          if (index >= 0 && index <= MAX_ARRAY_SIZE) {
            ...
            throw new IllegalArgumentException("index number out of range: " + index);
          }
        }
  ...
  public static float[] parseFloatArray(Map<String, String> traits, int elementCount, String key) {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINEST, "Unable to parse float; key: {0}; value: {1}", new Object[] {key, value});
    }
  ...
  public static String[] parseStringArray(
      Map<String, String> traits, int elementCount, String key) {
    String value = traits.get(key);
    String[] result = null;
    try {
      if (value != null) {
        ...
        for (int i = 0; i < lines.length; i++) {
          ...
          if (index >= 0 && index <= MAX_ARRAY_SIZE) {
            ...
            String val;
            if (tks.length > 1) {
              val = line.substring(line.indexOf(tks[0]) + tks[0].length()).trim();
      ...
      LOGGER.log(
          Level.FINEST, "Unable to parse String; key: {0}; value: {1}", new Object[] {key, value});
    }
