...
public class ScreenParser extends EDLParser {
  ...
  /**
   * We want embedded screens to NOT repeat IDs - so we share the same counter across multiple parse
   * method calls.
   */
  ...
  public Screen parse(URL url, ColorPalette colorList, int recursionLevel)
    ...
    try {
        ...
        while (scanner.hasNextLine()) {
          ...
          if (tokens.length > 0) {
            try {
              switch (tokens[0]) {
                ...
                case "object":
                  ...
                  if (className == null) {
                    ...
                    try {
                      ...
                      LOGGER.log(
                          Level.WARNING, "EDM Class definition not in classpath: {0}", className);
                      obj = new UnknownWidget();
                ...
                case "beginObjectProperties":
                  if (traits == null) { // additional properties! (RegTextupdateClass for example)
                    traits = last.getTraits(); // Hack, alternatively we could create new and update
                    // parseTraits to be addTraits?
                  }
                  break;
                    ...
                    if (last instanceof RelatedDisplay) {
                      ...
                      if (related.numDsps > 0) {
                        for (int i = 0; i < related.displayFileName.length; ++i) {
                          final URL relative =
                              EDLParser.getRelativeURL(url, related.displayFileName[i]);
                          if (relative != null)
                      ...
                      if (relative != null) image.file = relative.toExternalForm();
                    }
                  }

                  traits = null;
                  // last = null; // We can no longer clear last obj since widgets like
                  // RegTextupdateClass have multiple sets of properties
                  break;
                case "endGroup":
                  last = groupStack.pop();
                  ...
                  if (!line.isEmpty()) { // Skip blank lines
                    ...
                    if (line.trim().startsWith("#")) {
                      ...
                      if (line.trim().endsWith("{")) {
                        ...
                        value = stripQuotes(line.substring(tokens[0].length()));
              ...
              LOGGER.log(
                  Level.WARNING,
                  "Unable to parse line '" + line + "'; file '" + canonicalPath + "'; ignoring",
                  e);
            }
          }
        } // end while line
      ...
      if (edl_conn instanceof HttpURLConnection) {
        ((HttpURLConnection) edl_conn)
            .disconnect(); // If remote resource don't forget to disconnect!
      }
    }
        ...
        try {
          ...
          if (embedded instanceof ActiveSymbol) {
            if (embedded.file != null) {
              Screen s =
                  this.parse(
                      EDLParser.getURL(url, embedded.file, true), colorList, recursionLevel + 1);
            ...
            if (embedded.file != null && "file".equals(embedded.displaySource)) {
              ...
              embedded.screen = s;
                ...
                .equals(embedded.displaySource)) { // Use filePv to determine which menu item to use
              if (embedded.numDsps > 0 && embedded.numDsps <= TraitParser.MAX_ARRAY_SIZE) {
                for (int i = 0; i < embedded.numDsps; i++) {
                  ...
                  if (f != null) {
                    try {
                      Screen s2 =
                          this.parse(EDLParser.getURL(url, f, true), colorList, recursionLevel + 1);
                      ...
                      LOGGER.log(
                          Level.WARNING,
                          "Unable to load embedded menu file from: " + canonicalPath,
                          e);
                    }
                  }
                }
