...
public class TextScreenObject extends HtmlScreenObject {
  ...
  protected void set3DStyles() {
    ...
    if (!("ActiveButton".equals(className)
            || "ActiveMessageButton".equals(className)
            || "ActiveXTextDsp".equals(className))
        || threeDimensional) {
  ...
  public String toHtml(String indent, Point translation) {
    ...
    if (useAlarmBorder) {
      ...
      // In EDM alarm border is drawn right on top of existing widget instead of making space for it
      // like typical borders (part of width/height of obj).
      // We could have had an overlay to fake this, but instead we use negative margins and a border
      // on the text wrap element.
