...
public abstract class CoreWidget implements WEDMWidget {
  ...
  protected void setCommonAttributes() {
    ...
    classes.add(
        "MouseSensitive"); // Everything is MouseSensitive now that we must show tooltips on middle
    // mouse click
  ...
  public String getAttributesString(Map<String, String> attributes) {
    ...
    if (attributes != null && !attributes.isEmpty()) {
      ...
      attributesStr =
          key
              + "=\""
              + org.apache.taglibs.standard.functions.Functions.escapeXml(attributes.get(key))
              + "\"";
