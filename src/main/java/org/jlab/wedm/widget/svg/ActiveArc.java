...
public class ActiveArc extends SvgScreenObject {
  ...
  public String toSvg(String indent, Point translation) {
    ...
    // svg = svg + "<rect x=\"" + x + "\" y=\"" + y + "\" width=\"" + w + "\" height=\"" + h + "\"
    // stroke=\"red\" fill=\"transparent\" transform=\"rotate(" + startAngle + " " + (x + (w / 2)) +
    // " " + (y + (h / 2)) + ")\"/>\n";
    // svg = svg + "<ellipse cx=\"" + (x + (w / 2)) + "\" cy=\"" + (y + (h / 2)) + "\" rx=\"" + rx +
    // "\" ry=\"" + ry + "\" stroke=\"red\" fill=\"transparent\" transform=\"rotate(" + startAngle +
    // " " + (x + (w / 2)) + " " + (y + (h / 2)) + ")\" stroke-dasharray=\"" + DASH_SPACING +
    // "\"/>\n";
    return svg;
  }
}
