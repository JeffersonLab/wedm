...
public class ActiveLine extends SvgScreenObject {
  ...
  public String toSvg(String indent, Point translation) {
    ...
    if (xPoints != null
      ...
      // 1. SVG markers do not inherit line color (SVG 1.2 propsoal has fill="content-stroke", but
      // it is currently unsupported in all browsers
      // 2. SVG markers scale at a linear factor based on line width (looks like default ~ 1:1, but
      // w/viewBox and markerHeight/Width attr can change factor, but only linearly - also option
      // for don't scale at all exists w/markerUnits "userSpaceOnUse").  Linear scaling does NOT
      // match EDM, which is less than linear scaling
      if (startArrow) {
        float scaleFactor = 1;
