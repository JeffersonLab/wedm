...
public abstract class SvgScreenObject extends CoreWidget {
  ...
  public String startSvg(String indent, Point translation) {
    ...
    // and for SVG elements border: is placed in middle of dimensions (half of it is inside, half
    // outside).
    // EDM seems to be widget-specific
    // For HTML we could use CONTENT-BOX (border is drawn outside dimensions completely).
    // For SVG we could try to control box model / stroke location to be like CONTENT-BOX or
    // BORDER-BOX, but it isn't easy:
    ...
    String html = indent + "<svg " + attrStr + " " + classStr + " " + styleStr + ">\n";
